...
- 业务库 SQL 实现尚未完成对应的 DDL 探查（参见 `SPEC.md §5` 待联调事项）
...
- 任务状态流转条件未定义
- 接口入参或返回结构未定义
- 业绩聚合规则的边界未定义（拆单的某种状态、汇率回退后的下一步等）
- 业务库实际表结构是否与 `docs/prd/业务逻辑文档.md` 一致（在 DDL 探查完成前）
...
### 基本要求
- 任务状态必须显式定义，不允许隐式推断
- 不允许跳过必要状态（如 `<INITIAL> → <STAGE_2>`、`<INITIAL> → <TERMINAL_OK>`）
...
### 凭证处理
- GitLab token 通过临时 `credential.helper` 注入，**禁止**写入 `.git/config`
...
| 修改的内容 | 必须同步更新 |
|---|---|
| `<修改 A，例如：任务状态枚举 / 转移规则>` | `<同步 X，例如：SPEC.md §4 / 测试 / ACCEPTANCE.md §3>` |
| `<修改 B>`（按项目实际补全）| `<同步 Y>` |
| ……（按项目自身的文档体系补全）| |
